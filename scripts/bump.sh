...
#
# Edits `[workspace.package].version` in `Cargo.toml` (the single
# source of truth — every plugin in `plugins/*` inherits via
# `version.workspace = true`), refreshes `Cargo.lock`, and commits
# the result on whatever branch you're currently on. No fetch, no
# push — `scripts/release-macos.sh` does the tag-and-push.
...
#
# `[workspace.package].version` is the canonical source. Every plugin's
# Cargo.toml in `plugins/*` uses `version.workspace = true` and
# inherits it, so editing this one field moves the whole workspace.
...
# Rewrite the version line under `[workspace.package]` only. A blunt
# global s/"$CURRENT"/"$NEW"/ would also catch the `truce = { version
# = "..." }` line in `[workspace.dependencies]` if it ever happened
# to share the workspace version number — which is a different axis
# (truce framework version vs. our release version) and must not be
# touched here. Anchor to the [workspace.package] section instead.
echo "→ editing Cargo.toml"
sed_inplace "/^\[workspace\.package\]/,/^\[/ s/^version = \"$CURRENT\"/version = \"$NEW\"/" Cargo.toml
...
' Cargo.toml)"
if [[ "$POST" != "$NEW" ]]; then
    echo "Error: post-edit version is '$POST', expected '$NEW' (sed did not rewrite [workspace.package].version)" >&2
    exit 1
fi
