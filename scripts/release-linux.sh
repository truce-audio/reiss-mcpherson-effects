#!/usr/bin/env bash
# Release driver for the Linux half of a reiss-mcpherson-effects
# release. Builds one `.tar.gz` per plugin via `cargo truce package`
# and attaches them to the GitHub release created by
...
# No signing — Linux ships unsigned tarballs with an `install.sh`
...
# LV2 + standalone) and produces a `.tar.gz` per plugin with an
# `install.sh` that copies the bundles into the user's plugin
# directories. No `--formats` flag, no signing. With no `-p`,
# `cargo truce package` iterates every plugin in `truce.toml`.
echo "==> packaging"
cargo truce package
...
echo "==> built ${#tarball_paths[@]} tarball(s):"
for t in "${tarball_paths[@]}"; do echo "    $t"; done

# --- upload --------------------------------------------------------------

echo "==> uploading $(printf '%s ' "${tarball_paths[@]##*/}")to release $release_tag"
gh release upload "$release_tag" "${tarball_paths[@]}" --clobber
